# Commercial Legal Local Configuration Template

Copy this file to `config/local/codex-for-legal/commercial-legal/CLAUDE.md` before using practice-specific review workflows.
...
4. On first run after a plugin update, if a populated CLAUDE.md exists at the old cache path
   (config/local/cache/codex-for-legal/commercial-legal/<version>/CLAUDE.md for any version)
...
|---|---|---|
| CLM (Ironclad (connector optional; not enabled in v1), Agiloft, etc.) | [PLACEHOLDER ✓/✗] | Manual record-keeping; renewal-tracker runs against a local register |
| E-signature (DocuSign (connector optional; not enabled in v1), etc.) | [PLACEHOLDER ✓/✗] | User routes for signature outside the plugin |
| Document storage (Drive / SharePoint / Box) | [PLACEHOLDER ✓/✗] | User uploads agreements directly for each review |
| Slack (connector optional; not enabled in v1) | [PLACEHOLDER ✓/✗] | Alerts and stakeholder summaries delivered inline instead of posted |
...
|---|---|---|---|
| [Paralegal/junior] | [PLACEHOLDER threshold] | [Counsel] | [Slack (connector optional; not enabled in v1)/email] |
...
**Where work product goes:** [PLACEHOLDER — CLM, Drive folder, Slack (connector optional; not enabled in v1) channel]

**Renewal alerts go to:** [PLACEHOLDER — Slack (connector optional; not enabled in v1) channel or email]
...
**Verification log.** When you or the user verifies a flagged item — confirms a cite against a primary source, checks a deadline against the local rule, verifies a threshold against the current statute — record it so the next person doesn't re-verify. Write a one-line entry to `config/local/codex-for-legal/commercial-legal/verification-log.md`:
...
When the user asks a question in this plugin's practice area — not just when they invoke a skill — read the practice profile at `config/local/codex-for-legal/commercial-legal/CLAUDE.md` (and `config/local/codex-for-legal/company-profile.md`) first, and apply it. If it's populated, answer as the configured assistant:
