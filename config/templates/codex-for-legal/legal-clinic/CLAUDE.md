# Legal Clinic Local Configuration Template

Copy this file to `config/local/codex-for-legal/legal-clinic/CLAUDE.md` before using practice-specific review workflows.
...
4. On first run after a plugin update, if a populated CLAUDE.md exists at the old cache path
   (config/local/cache/codex-for-legal/legal-clinic/<version>/CLAUDE.md for any version)
...
| Document storage (Google Drive (connector optional; not enabled in v1) / SharePoint / Box) | [✓ / ✗] | Student outputs save to local filesystem; review stays in-plugin |
...
The supervisor can author a per-practice-area guide at `config/local/codex-for-legal/legal-clinic/guides/<practice-area>.md`. Student-facing skills read the guide before doing substantive work. The guide controls:
...
**Verification log.** When you or the user verifies a flagged item — confirms a cite against a primary source, checks a deadline against the local rule, verifies a threshold against the current statute — record it so the next person doesn't re-verify. Write a one-line entry to `config/local/codex-for-legal/legal-clinic/verification-log.md`:
...
**Deadlines file:** `config/local/codex-for-legal/legal-clinic/deadlines.yaml` (populated by `/deadlines --add`)
...
When the user asks a question in this plugin's practice area — not just when they invoke a skill — read the practice profile at `config/local/codex-for-legal/legal-clinic/CLAUDE.md` (and `config/local/codex-for-legal/company-profile.md`) first, and apply it. If it's populated, answer as the configured assistant:
