# Corporate Legal Local Configuration Template

Copy this file to `config/local/codex-for-legal/corporate-legal/CLAUDE.md` before using practice-specific review workflows.
...
4. On first run after a plugin update, if a populated CLAUDE.md exists at the old cache path
   (config/local/cache/codex-for-legal/corporate-legal/<version>/CLAUDE.md for any version)
...
|---|---|---|
| VDR (Intralinks, Datasite, Box) | [✓ / ✗] | Diligence pulls from local folder; user drops docs in `config/local/codex-for-legal/corporate-legal/deals/[code]/vdr-mirror/` |
| Board portal (Diligent, BoardEffect) | [✓ / ✗] | Minutes/consents work from local templates; no portal posting |
| Document storage (Google Drive (connector optional; not enabled in v1), SharePoint, Box) | [✓ / ✗] | Read local paths; no cross-system search |
| Slack (connector optional; not enabled in v1) | [✓ / ✗] | Briefs emitted as files only; no in-channel summaries |
...
**Verification log.** When you or the user verifies a flagged item — confirms a cite against a primary source, checks a deadline against the local rule, verifies a threshold against the current statute — record it so the next person doesn't re-verify. Write a one-line entry to `config/local/codex-for-legal/corporate-legal/verification-log.md`:
...
When the user asks a question in this plugin's practice area — not just when they invoke a skill — read the practice profile at `config/local/codex-for-legal/corporate-legal/CLAUDE.md` (and `config/local/codex-for-legal/company-profile.md`) first, and apply it. If it's populated, answer as the configured assistant:
...
**Format:** [PLACEHOLDER — email / Slack (connector optional; not enabled in v1) / call]
...
**Consents repository:** [PLACEHOLDER — folder path / Google Drive (connector optional; not enabled in v1) / SharePoint / Box location, or "seed documents only"]
**Consent format:**
...
**Compliance tracker:** `config/local/codex-for-legal/corporate-legal/entities/compliance-tracker.yaml`
