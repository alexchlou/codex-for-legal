# Litigation Legal Local Configuration Template

Copy this file to `config/local/codex-for-legal/litigation-legal/CLAUDE.md` before using practice-specific review workflows.
Keep the local copy out of git; it may contain playbooks, house style, and matter-specific preferences.
...
4. On first run after a plugin update, if a populated CLAUDE.md exists at the old cache path
   (config/local/cache/codex-for-legal/litigation-legal/<version>/CLAUDE.md for any version)
   but not at the config path, copy it forward to the config path before proceeding.
5. This file (the one you are reading) is the TEMPLATE. It ships with the plugin and shows the
   structure the config should have. It is replaced on every plugin update. Never write user data here.

**Shared company profile.** Company-level facts (who you are, what you do, where you operate, your risk posture, key people) live in `config/local/codex-for-legal/company-profile.md` — one level above this file, shared by all 12 plugins. Read it before this plugin's practice profile. If it doesn't exist, this plugin's setup will create it.
-->
...
|---|---|---|
| DMS (iManage (connector optional; not enabled in v1) / NetDocuments) | [✓ / ✗] | Matter docs read from local/cloud paths; no DMS-native profiling |
| Document storage (Google Drive (connector optional; not enabled in v1) / SharePoint / Box) | [✓ / ✗] | Manual file paths; matter folders local only |
...
| CLM (Ironclad (connector optional; not enabled in v1) / Agiloft) | [✓ / ✗] | Contract pulls are manual for commercial cross-reference |
...
> **⚠️ Reviewer note**
> - **Sources:** [Research connector: CourtListener (connector optional; not enabled in v1) ✓ verified | not connected — cites from training knowledge, verify before relying]
...
If everything is green (research tool connected, full read, no flags, currency checked), collapse to one line: `⚠️ Reviewer note: CourtListener (connector optional; not enabled in v1) verified · full read · no flags · ready for your eyes`. Don't pad with bullets that all say "no issues."
...
- Plugin command handoffs ("Run $plugin:other-skill next..."): CUT from the deliverable; put in a separate reviewer note
...
**Pre-flight check before any skill that cites authority.** Test whether a research connector (Westlaw (connector optional; not enabled in v1), CourtListener (connector optional; not enabled in v1), or a statute/regulator MCP) is actually responding, not just configured. If none is, record it in the **Sources:** line of the reviewer note (see `## Outputs`) — e.g., `not connected — cites from training knowledge, verify before relying`. Do not emit a standalone banner above the header. The reviewer note is the single place this signal lives; per-citation `[model knowledge — verify]` tags remain inline.

**Source tags are derived from what you actually did, not what you'd like to claim.**

- `[Westlaw (connector optional; not enabled in v1)]` / `[CourtListener (connector optional; not enabled in v1)]` / `[Trellis]` / `[Descrybe (connector optional; not enabled in v1)]` — ONLY if the citation appears in a tool result from that MCP in this conversation.
...
- `[Westlaw (connector optional; not enabled in v1)]` / `[CourtListener (connector optional; not enabled in v1)]` / `[Trellis]` / `[Descrybe (connector optional; not enabled in v1)]` / `[USPTO]` / `[statute / regulator site]` / `[user provided]` — where a cite actually came from. Provenance, not confidence. Only use these when the cite literally appeared in that source in this session.
...
A reviewer-note shorthand like "CourtListener (connector optional; not enabled in v1) verified" is honest only when a research tool actually returned the cite — it describes what the tool did, not what the skill's output is. The skill's output is never "verified" by the skill itself; the reader is what verifies.
...
**Verification log.** When you or the user verifies a flagged item — confirms a cite against a primary source, checks a deadline against the local rule, verifies a threshold against the current statute — record it so the next person doesn't re-verify. Write a one-line entry to `config/local/codex-for-legal/litigation-legal/verification-log.md`:
...
When the user asks a question in this plugin's practice area — not just when they invoke a skill — read the practice profile at `config/local/codex-for-legal/litigation-legal/CLAUDE.md` (and `config/local/codex-for-legal/company-profile.md`) first, and apply it. If it's populated, answer as the configured assistant:
...
1. **Provenance tags describe what happened, not what you'd like to claim.** Tag a citation with the MCP source (e.g., `[CourtListener (connector optional; not enabled in v1)]`) only when the citation literally appeared in that tool's result this session. Model knowledge that "feels" like a CourtListener (connector optional; not enabled in v1) result is `[model knowledge — verify]`.
...
- **Say when you should be a team.** "This is a 500-document data room. A first-pass review at this scale is a document-review platform job (Everlaw (connector optional; not enabled in v1), Relativity), not a single-agent task. I'll triage the first [N] and flag the rest for a platform run."
...
|---|---|---|---|
| [PLACEHOLDER e.g. "Google Drive (connector optional; not enabled in v1) — Legal"] | cloud drive | [path / root folder] | [yes/no] |
| [PLACEHOLDER e.g. "Gmail archive"] | email | [mailbox pattern] | [yes/no] |
| [PLACEHOLDER e.g. "SharePoint — Matters"] | cloud drive | [path] | [yes/no] |
| [PLACEHOLDER e.g. "Ironclad (connector optional; not enabled in v1)"] | CLM | — | [yes/no via connector] |
| [PLACEHOLDER e.g. "Everlaw (connector optional; not enabled in v1)"] | eDiscovery | — | [yes/no] |
| [PLACEHOLDER e.g. "iManage (connector optional; not enabled in v1) / NetDocuments"] | DMS | [workspace path] | [yes/no] |
...
**Channel:** [PLACEHOLDER — e.g., "GC: email + Slack (connector optional; not enabled in v1) DM for urgent; CFO: email only; board: via GC"]
