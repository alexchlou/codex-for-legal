...
1. Load `config/local/codex-for-legal/privacy-legal/CLAUDE.md` → PIA house style (trigger, structure, depth, sign-off).
...
```
$privacy-legal:pia-generation
PRD: [local file path or pasted text (Drive requires a separately configured connector)]
```
...
Before writing a new PIA, check the outputs folder for prior work on the same feature, processing activity, or counterparty. Read `config/local/codex-for-legal/privacy-legal/CLAUDE.md` → `## Outputs` for the path. Scan for:
...
**Carry severity from upstream as a floor** per the cross-skill severity floor rule in `config/local/codex-for-legal/privacy-legal/CLAUDE.md` → `## Shared guardrails`. A use-case-triage that rated the activity high-risk cannot become a PIA that concludes low-risk without stating why and what changed.
...
Read `config/local/codex-for-legal/privacy-legal/CLAUDE.md` → `## PIA house style`. That has:
...
Check the trigger criteria in `config/local/codex-for-legal/privacy-legal/CLAUDE.md`. That is the team's house answer.
...
**Use the seed PIA structure from the config CLAUDE.md.** If none was captured, use this default. Prepend the work-product header from `config/local/codex-for-legal/privacy-legal/CLAUDE.md` `## Outputs` (it differs by user role — see `## Who's using this`).
...
Every PIA should cross-check against the privacy policy commitments in `config/local/codex-for-legal/privacy-legal/CLAUDE.md`. The common drift:
...
- **To the sign-off process:** Per `config/local/codex-for-legal/privacy-legal/CLAUDE.md` → who approves PIAs.
...
**Before proceeding to submit a DPIA (or any equivalent impact assessment) to a regulator, supervisory authority, or enforcement body:** Read `## Who's using this` in `config/local/codex-for-legal/privacy-legal/CLAUDE.md`. If the Role is Non-lawyer:
