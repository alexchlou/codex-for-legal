...
1. Load `config/local/codex-for-legal/privacy-legal/CLAUDE.md` → DSAR process (systems list, verification method, SLA).
...
Read `config/local/codex-for-legal/privacy-legal/CLAUDE.md` → `## DSAR process`. That section has:
...
Per the method in `config/local/codex-for-legal/privacy-legal/CLAUDE.md`. Common approaches:
...
Walk the systems list from `config/local/codex-for-legal/privacy-legal/CLAUDE.md`. For each system:
...
**Before proceeding to send either letter to the data subject:** Read `## Who's using this` in `config/local/codex-for-legal/privacy-legal/CLAUDE.md`. If the Role is Non-lawyer:
...
> **Note:** Both DSAR letters are externally-facing deliverables sent to the data subject. Do **not** include the work-product header from `config/local/codex-for-legal/privacy-legal/CLAUDE.md` `## Outputs` on either letter. Internal notes, logs, and exemption analyses that accompany the letters are attorney work product — keep those separate and prepend the work-product header per `config/local/codex-for-legal/privacy-legal/CLAUDE.md` `## Outputs` (which differs by user role — see `## Who's using this`).
...
Per `config/local/codex-for-legal/privacy-legal/CLAUDE.md` → Escalation table, escalate when:
...
If `config/local/codex-for-legal/privacy-legal/CLAUDE.md` → `## DSAR process` records an internal SLA that is tighter than the legal deadline, use the internal SLA and note the legal backstop.
