...
1. Read `config/local/codex-for-legal/privacy-legal/CLAUDE.md` → outputs folder path, policy document, last sweep date.
...
6. Update Last policy sweep date in `config/local/codex-for-legal/privacy-legal/CLAUDE.md`.

**Direct query mode** (with description argument):
1. Read `config/local/codex-for-legal/privacy-legal/CLAUDE.md` → current policy commitments + actual policy document.
...
Update **Last policy sweep** in `config/local/codex-for-legal/privacy-legal/CLAUDE.md` to today's date after completing the sweep.
...
`config/local/codex-for-legal/privacy-legal/CLAUDE.md`, so the next sweep only looks at new files.
...
- It doesn't enforce that outputs are saved — if the team isn't saving PIAs to the
  ...
  saved outputs.
- It doesn't read email or Slack (connector optional; not enabled in v1) for informal decisions — only structured outputs
