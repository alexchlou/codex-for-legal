...
1. **Follow the workflow below** and read
   `config/local/codex-for-legal/ip-legal/portfolio.yaml`.
...
Lives at `config/local/codex-for-legal/ip-legal/portfolio.yaml`.
Structure:
...
Output (prepend work-product header per `config/local/codex-for-legal/ip-legal/CLAUDE.md` → Outputs):
...
`## Who's using this` in `config/local/codex-for-legal/ip-legal/CLAUDE.md`. If the Role is **Non-lawyer**:
