...
1. Read `config/local/codex-for-legal/ip-legal/CLAUDE.md`. If it
...
5. Similar-marks search against what's connected (Solve Intelligence, CourtListener (connector optional; not enabled in v1), Descrybe (connector optional; not enabled in v1), or whatever MCP is available). If nothing is
...
- **Integrations** from `## Available integrations` (CourtListener (connector optional; not enabled in v1) / Solve Intelligence / Descrybe (connector optional; not enabled in v1) — each determines what searches are available to run, what the fallback is, and what gets attributed in the output).
- **Decision posture** from `## Decision posture on subjective legal calls` — this skill never concludes "not confusingly similar."

If `config/local/codex-for-legal/ip-legal/CLAUDE.md` contains `[PLACEHOLDER]` or `[Your Company Name]`, surface this bounce:
...
Read `## Available integrations` from `config/local/codex-for-legal/ip-legal/CLAUDE.md`:

- **If a trademark search connector is available** (Solve Intelligence,
  Descrybe (connector optional; not enabled in v1) — or any MCP exposing TM-registry search): run a preliminary search
...
- **If a legal research connector is available** (CourtListener (connector optional; not enabled in v1) for litigation for case law and TTAB decisions): sweep for reported disputes involving
...
> Intelligence, Descrybe (connector optional; not enabled in v1), CourtListener (connector optional; not enabled in v1), state registries, Madrid/WIPO, or any
...
Per the decision posture in `config/local/codex-for-legal/ip-legal/CLAUDE.md`:
...
Prepend the work-product header from `config/local/codex-for-legal/ip-legal/CLAUDE.md` `## Outputs`.
...
TESS, Solve Intelligence, Descrybe (connector optional; not enabled in v1), CourtListener (connector optional; not enabled in v1), state registries,
...
- [routing per `config/local/codex-for-legal/ip-legal/CLAUDE.md` —
...
`config/local/codex-for-legal/ip-legal/matters/<matter-slug>/outputs/clearance-<mark-slug>-YYYY-MM-DD.md`.
Otherwise write to
`config/local/codex-for-legal/ip-legal/outputs/clearance-<mark-slug>-YYYY-MM-DD.md`
