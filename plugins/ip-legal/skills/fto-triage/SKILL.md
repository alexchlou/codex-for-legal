...
Before running triage, read `config/local/codex-for-legal/ip-legal/CLAUDE.md`. Pull:
...
- **Standards-essential declarations** (IEEE, ETSI, 3GPP) if the product touches
  a relevant standard.
- **Reported litigation** in the technology space (CourtListener (connector optional; not enabled in v1) / RECAP, Unified
  Patents, Lex Machina).
...
Prepend the work-product header from `config/local/codex-for-legal/ip-legal/CLAUDE.md` `## Outputs`. Mark the document as privileged if the role is lawyer; see the non-lawyer gate below if not.
...
`config/local/codex-for-legal/ip-legal/matters/<matter-slug>/outputs/fto-triage-<subject-slug>-YYYY-MM-DD.md`.
Otherwise write to
`config/local/codex-for-legal/ip-legal/outputs/fto-triage-<subject-slug>-YYYY-MM-DD.md`
