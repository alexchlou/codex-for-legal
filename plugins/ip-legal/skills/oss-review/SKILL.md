...
**Before scanning dependencies, read `config/local/codex-for-legal/ip-legal/CLAUDE.md`.** If it is missing or still contains placeholders, stop and run `$ip-legal:cold-start-interview`. The practice profile tells you:
...
Prepend the work-product header from `config/local/codex-for-legal/ip-legal/CLAUDE.md` → `## Outputs` (differs by user role — see `## Who's using this`).
