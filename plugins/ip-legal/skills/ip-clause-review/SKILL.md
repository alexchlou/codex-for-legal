...
argument-hint: "[file path | local file path or pasted text (Drive requires a separately configured connector) | paste text]"
---
...
Reviews the IP clauses in an agreement against the practice profile in `config/local/codex-for-legal/ip-legal/CLAUDE.md`. Flags assignment gaps, ownership ambiguity, license-scope issues, and IP warranty/indemnity problems. Produces a memo with per-clause findings, prioritized by risk, with suggested redline language where appropriate.

## Instructions

1. **Load `config/local/codex-for-legal/ip-legal/CLAUDE.md`.** If the file is missing or placeholders are present, stop and prompt: "Run `$ip-legal:cold-start-interview` first — I need to learn your practice profile before I can review IP clauses against it."

2. **Get the agreement:** From file path, local file path or pasted text (Drive requires a separately configured connector), or pasted text. If none provided, ask.
...
**Before reading the agreement, read `config/local/codex-for-legal/ip-legal/CLAUDE.md`.** If it is missing or still contains placeholders, stop and run `$ip-legal:cold-start-interview`. The practice profile tells you:
...
**Escalation:** Per `config/local/codex-for-legal/ip-legal/CLAUDE.md`, assignment-scope gaps escalate to [approver].
```
...
Prepend the work-product header from `config/local/codex-for-legal/ip-legal/CLAUDE.md` → `## Outputs` (it differs by user role — see `## Who's using this`).
