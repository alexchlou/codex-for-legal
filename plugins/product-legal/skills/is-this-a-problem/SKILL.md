...
description: >
  Fast "is this a problem?" answer for the quick Slack (connector optional; not enabled in v1) question — pattern-matches
...
1. Load `config/local/codex-for-legal/product-legal/CLAUDE.md` → Risk calibration.
...
Most "quick legal question" Slack (connector optional; not enabled in v1)s are one of three things: (a) not a problem, say so fast, (b) a real thing that needs a real look, route it, (c) a thing that looks fine but has a trap, catch the trap. This skill sorts in under a minute using the calibration table.

The goal is speed. The PM asked at 4:47pm. They want an answer, not a memo.

## Load calibration

Read `config/local/codex-for-legal/product-legal/CLAUDE.md` → `## Risk calibration`. The whole point of this skill is pattern-matching against that table.
...
**For Slack (connector optional; not enabled in v1) (the common case):**

Slack (connector optional; not enabled in v1) triage replies are internal legal advice. If the reply is being pasted into a ticket, document, or channel that's broadly shared with non-legal, prepend the work-product header from `config/local/codex-for-legal/product-legal/CLAUDE.md` `## Outputs` (it differs by user role — see `## Who's using this`):

```
[WORK-PRODUCT HEADER — per plugin config ## Outputs]
```

For an in-the-flow Slack (connector optional; not enabled in v1) DM reply to the PM, the short form is:
