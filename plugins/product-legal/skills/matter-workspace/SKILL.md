...
**Default state is off.** In-house users never see this — they run at practice-level only. Matter workspaces turn on at cold-start for private-practice users, or by editing `## Matter workspaces` in the practice-level CLAUDE.md. If `Enabled` is `✗`, this skill does not run; the `$product-legal:matter-workspace` command explains the disabled state and suggests `$product-legal:cold-start-interview --redo` for users who actually need matter isolation.
...
```
config/local/codex-for-legal/product-legal/
