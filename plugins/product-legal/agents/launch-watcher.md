...
Run daily. Set a morning reminder (calendar block, cron, or team ritual) to invoke the launch-watcher — Codex agents do not self-schedule. Pulls tickets with launch dates in the next 30 days.

**Slack (connector optional; not enabled in v1) delivery:** Posting the digest to Slack (connector optional; not enabled in v1) requires a Slack (connector optional; not enabled in v1) MCP server configured in your environment. If no Slack (connector optional; not enabled in v1) MCP is available, write the digest to a file (e.g., `launch-radar-[date].md`) instead — the filtering logic is independent of the delivery path.

## What it does

1. Read `config/local/codex-for-legal/product-legal/CLAUDE.md` → launch tracker location, calibration table, escalation channel.
