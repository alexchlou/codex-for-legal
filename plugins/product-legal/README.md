...
Your configuration is stored at `config/local/codex-for-legal/product-legal/CLAUDE.md` and survives plugin updates.
...
| Skill | Purpose |
|---|---|
| **cold-start-interview** | Writes config/local/codex-for-legal/product-legal/CLAUDE.md from interview + past launch reviews |
...
| **is-this-a-problem** | Same-minute triage for the quick Slack (connector optional; not enabled in v1) question |
...
Your practice profile at `config/local/codex-for-legal/product-legal/CLAUDE.md` isn't static — it improves as you use the plugin. Skills tell you when an output used a default you should tune. You can re-run setup, edit the file directly, or tell a skill to record a new position.
...
Your configuration is stored at `config/local/codex-for-legal/product-legal/CLAUDE.md` and survives plugin updates — you only run setup once.
