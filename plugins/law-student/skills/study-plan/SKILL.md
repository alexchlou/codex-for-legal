...
1. Load `config/local/codex-for-legal/law-student/CLAUDE.md` → bar jurisdiction, exam format, bar date, weak subjects, target study hours/day, prep course.
2. Load `config/local/codex-for-legal/law-student/study-plan.yaml` if it exists.
...
`config/local/codex-for-legal/law-student/study-plan.yaml` if it exists — extend, don't overwrite.
...
Write to `config/local/codex-for-legal/law-student/study-plan.yaml`:
