...
1. Load `config/local/codex-for-legal/law-student/CLAUDE.md` → current classes, weak subjects, outline locations.
2. Apply the framework below.
3. Route by flag:
   - `--generate`: build cards from source (outline path, notes, casebook) per card-writing rules. Write to `config/local/codex-for-legal/law-student/flashcards/[subject]/cards.md`.
...
- `config/local/codex-for-legal/law-student/CLAUDE.md` → current classes, weak subjects, existing outlines
- `config/local/codex-for-legal/law-student/flashcards/[subject]/cards.md` if it exists (incremental build)
...
- Load `config/local/codex-for-legal/law-student/study-plan.yaml` if it exists and read `session_history` for this subject.
...
- If no `study-plan.yaml`, write to `config/local/codex-for-legal/law-student/session-history.yaml` instead.

### `--generate` — create cards

**Inputs:**
- Subject (class name or topic)
- Source (outline path, notes, or "use my existing outline from config/local/codex-for-legal/law-student/CLAUDE.md")
...
**Citation check.** When cards are generated from my knowledge rather than a source you pasted, the rule and any case/statute cited on the back were generated by an AI model and have not been verified. Before you memorize a card, confirm it against your outline, casebook, or a research tool (Westlaw (connector optional; not enabled in v1), Fastcase, CourtListener (connector optional; not enabled in v1)). A wrong card drilled to mastery is worse than no card.
