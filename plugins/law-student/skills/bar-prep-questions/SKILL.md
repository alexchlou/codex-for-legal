...
1. Load `config/local/codex-for-legal/law-student/CLAUDE.md` → bar jurisdiction, exam format (NextGen / traditional UBE / state-specific), weak subjects, prep course.
2. Also load `config/local/codex-for-legal/law-student/study-plan.yaml` if it exists — it tells you what subject is scheduled for today and what subtopics are still weak.
...
1. Load `config/local/codex-for-legal/law-student/CLAUDE.md` and read the bar jurisdiction and bar date.
...
`config/local/codex-for-legal/law-student/CLAUDE.md` → bar jurisdiction, exam format (NextGen / traditional UBE / state-specific), weak subjects, prep course. If exam format isn't specified, run the "Exam type" gate above before continuing. If jurisdiction is specified, apply the `## Jurisdiction handling` rules — label questions by which rule body governs, and flag divergences explicitly.

Also load `config/local/codex-for-legal/law-student/study-plan.yaml` if it exists (written by the `study-plan` skill). If the plan has a session scheduled for today or specifies weak subjects to weight, honor it.

## Session mode

`--session <n>` runs a focused N-question session on a specific subject, tracks performance, and writes session results back to `config/local/codex-for-legal/law-student/study-plan.yaml` under `session_history` so the study plan adapts.
...
If no `study-plan.yaml` exists, write session history to `config/local/codex-for-legal/law-student/session-history.yaml` instead so future sessions can still weight appropriately.
...
Subject distribution: weight toward weak subjects **within the subjects actually tested on the student's exam**. If `config/local/codex-for-legal/law-student/CLAUDE.md` says weak on Evidence and Civ Pro, 60% of questions come from those.
