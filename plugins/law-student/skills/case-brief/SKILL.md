...
1. Load `config/local/codex-for-legal/law-student/CLAUDE.md` → outline/brief preferences.
...
`config/local/codex-for-legal/law-student/CLAUDE.md` → outline/brief preferences (format, depth), learning style.
...
Per the student's format in `config/local/codex-for-legal/law-student/CLAUDE.md`. If none captured, default:
...
**Citation check.** The case cite, quoted language, and any supporting authority above were generated by an AI model and have not been verified. Before you rely on them — in a brief, memo, outline entry, or exam answer — look them up on Westlaw (connector optional; not enabled in v1), Fastcase, CourtListener (connector optional; not enabled in v1), or your school's research tool. AI-generated citations are sometimes fabricated or misquoted.
```

## Depth calibration

Per `config/local/codex-for-legal/law-student/CLAUDE.md` — some students want one-line briefs (rule + cite), some want full treatment. Match their format.
