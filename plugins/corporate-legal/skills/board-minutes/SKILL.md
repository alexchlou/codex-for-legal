...
- `config/local/codex-for-legal/corporate-legal/CLAUDE.md` → `## Board & Secretary` section:
...
- If `config/local/codex-for-legal/corporate-legal/CLAUDE.md` has no minutes format: run cold-start first. Do not proceed with a generic format.
...
**Directors present:**
- Pull from board composition in `config/local/codex-for-legal/corporate-legal/CLAUDE.md` as the starting point
...
Use the house format from `config/local/codex-for-legal/corporate-legal/CLAUDE.md`. Do not default to a generic format. The seed minutes are the template — replicate the structure, the header, the resolution language, the level of discussion detail.
...
**Previous minutes:**
Standard language: approval of minutes from prior meeting. Pull date of prior meeting from `config/local/codex-for-legal/corporate-legal/CLAUDE.md` board calendar if available, otherwise leave as [DATE OF PRIOR MEETING].
...
RESOLVED, that [resolution text in house language from `config/local/codex-for-legal/corporate-legal/CLAUDE.md`].
```
...
**Before adopting minutes as final:** Read `## Who's using this` in `config/local/codex-for-legal/corporate-legal/CLAUDE.md`. If the Role is **Non-lawyer**:
...
Produce the full draft. The minutes themselves are a corporate record, not privileged; do not apply the work-product header to the minutes as circulated. The drafting notes, placeholder flags, and review checklist below are work product — prepend the work-product header from `config/local/codex-for-legal/corporate-legal/CLAUDE.md` `## Outputs` (it differs by user role — see `## Who's using this`):
