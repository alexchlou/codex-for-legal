...
1. Read `config/local/codex-for-legal/corporate-legal/deals/[code]/closing-checklist.yaml` and use the modes below.
...
Lives at `config/local/codex-for-legal/corporate-legal/deals/[code]/closing-checklist.yaml`. Structure:
...
The agent checks the checklist daily, pulls any status updates from email/Slack (connector optional; not enabled in v1) if connected, and posts the "what's blocking" report to the deal team channel. Mode 4 is the agent's output.

## Consequential-action gate (certify closing)

**Before producing a "ready to close / all CPs satisfied" certification or closing memo:** Read `## Who's using this` in `config/local/codex-for-legal/corporate-legal/CLAUDE.md`. If the Role is **Non-lawyer**:
