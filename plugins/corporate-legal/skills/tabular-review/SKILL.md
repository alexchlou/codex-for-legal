...
1. Load `config/local/codex-for-legal/corporate-legal/CLAUDE.md` → diligence structure, thresholds, house format.
...
- `config/local/codex-for-legal/corporate-legal/CLAUDE.md` → diligence structure, materiality thresholds, house format preferences
- `config/local/codex-for-legal/corporate-legal/deals/[code]/deal-context.md` if working a specific deal
...
Confirm:
1. **Documents.** Where are they? VDR MCP (Box, Datasite, iManage (connector optional; not enabled in v1)), local folder, Google Drive (connector optional; not enabled in v1) folder, or a list of files. How many? If >200, warn that this will take a while and offer to start with a materiality-filtered subset.
