...
description: >
  ...
  per the trust level in `config/local/codex-for-legal/corporate-legal/CLAUDE.md`. Use when user says "send to Luminance",
...
`config/local/codex-for-legal/corporate-legal/CLAUDE.md` → AI-assisted review:
...
If `config/local/codex-for-legal/corporate-legal/CLAUDE.md` says no AI tool → this skill is a no-op. Everything goes through diligence-issue-extraction directly.
...
- Specify extraction targets per `config/local/codex-for-legal/corporate-legal/CLAUDE.md` (which clause types)
- Note the materiality threshold so tool output can be filtered

### Step 2: Load (or instruct the loader)

Per `config/local/codex-for-legal/corporate-legal/CLAUDE.md` — who loads. If it's you, generate the load instructions. If it's someone else, generate the request:
...
- Exclusivity
- [etc. per `config/local/codex-for-legal/corporate-legal/CLAUDE.md`]
...
**"Use as-is":** Ingest directly into diligence findings. (Only if `config/local/codex-for-legal/corporate-legal/CLAUDE.md` says this — it's rare.)
...
**Trust level:** [per `config/local/codex-for-legal/corporate-legal/CLAUDE.md`]
...
- It doesn't replace the tool's output with its own judgment entirely — if `config/local/codex-for-legal/corporate-legal/CLAUDE.md` says spot-check 10%, check 10%, not 100%.
- It doesn't decide the trust level — that's in `config/local/codex-for-legal/corporate-legal/CLAUDE.md`, set at cold-start based on the team's experience with the tool.
