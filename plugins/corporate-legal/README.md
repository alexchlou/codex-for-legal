...
Walks through module selection, then a short targeted interview for each active area. Writes a modular `config/local/codex-for-legal/corporate-legal/CLAUDE.md` with only the relevant sections. Your configuration is stored at that path and survives plugin updates.
...
**Connect a research tool first — the citation guardrails depend on it.** Without one, every cite is tagged `[verify]` and the reviewer note above each deliverable records that sources weren't verified. Skills work either way; a research tool (CourtListener (connector optional; not enabled in v1)) just shifts verification work off your plate.

Ships with:

- **Slack (connector optional; not enabled in v1)** — search messages, read channels, find discussions (general bucket)
- **Google Drive (connector optional; not enabled in v1)** — search, read, and fetch documents (general bucket)
...
Your practice profile at `config/local/codex-for-legal/corporate-legal/CLAUDE.md` isn't static — it improves as you use the plugin. Skills tell you when an output used a default you should tune. You can re-run setup, edit the file directly, or tell a skill to record a new position.
...
- AI tool handoff (Luminance/Kira) is optional. If `config/local/codex-for-legal/corporate-legal/CLAUDE.md` says no tool, all extraction runs through the direct skill.
