...
1. Read `config/local/codex-for-legal/ai-governance-legal/CLAUDE.md`. Confirm registry is populated — if not, stop and direct to setup.
...
## Read `config/local/codex-for-legal/ai-governance-legal/CLAUDE.md` first

Before triaging, always read `config/local/codex-for-legal/ai-governance-legal/CLAUDE.md`. The use case registry and red lines there
are authoritative. Generic AI ethics reasoning is not a substitute for what this
company has actually decided.

If `config/local/codex-for-legal/ai-governance-legal/CLAUDE.md` contains `[PLACEHOLDER]`, surface this bounce:
...
Check the use case registry in `config/local/codex-for-legal/ai-governance-legal/CLAUDE.md` for a direct or close match.
...
The APPROVED / CONDITIONAL / NOT APPROVED buckets, the red-line definitions, and the CONDITIONAL required-controls list all come from `config/local/codex-for-legal/ai-governance-legal/CLAUDE.md` → `## AI use case triage criteria` and `## Use case registry`. If the playbook doesn't define a criterion the use case turns on, ask the user: "Your playbook doesn't cover [specific question]. What's your default position? I'll add it to `config/local/codex-for-legal/ai-governance-legal/CLAUDE.md` so the next triage is consistent."

**Before issuing an APPROVED classification (approving an AI use case for deployment):** Read `## Who's using this` in `config/local/codex-for-legal/ai-governance-legal/CLAUDE.md`. If the Role is Non-lawyer:
...
**Governance tier:** [Standard / Elevated / High — per `config/local/codex-for-legal/ai-governance-legal/CLAUDE.md`]
...
> "Add to `config/local/codex-for-legal/ai-governance-legal/CLAUDE.md` → Use case registry. This means next time the same request
