...
- **Flag substantial modification.** Whenever a system is modified beyond
  configuration, prompt the user to re-run `$ai-governance-legal:ai-inventory classify` —
  modification can change role.
- **Don't declare obligations from a table.** If asked, do the analysis
  in conversation and route to `$ai-governance-legal:aia-generation` for anything that needs
  a formal record.
