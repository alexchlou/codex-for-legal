...
Per-semester folder with one handoff memo per active case. Produced by `$legal-clinic:semester-handoff` at end of semester. Read by incoming students during `$legal-clinic:ramp` for the cases they inherit.
...
3. Incoming cohort runs `$legal-clinic:ramp` at start of next semester; `$legal-clinic:ramp` surfaces the handoff memos for cases each new student is assigned.
