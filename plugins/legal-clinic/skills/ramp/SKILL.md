...
1. Check `config/local/codex-for-legal/legal-clinic/CLAUDE.md` is set up. If placeholders: "Ask [professor] to run `$legal-clinic:cold-start-interview` first."
...
**Audience: students.** Professors don't run this (they run `$legal-clinic:cold-start-interview`).

## Load context

`config/local/codex-for-legal/legal-clinic/CLAUDE.md` → clinic profile, practice areas, jurisdiction, handbook path, supervision style, practice-area templates.

If that file is missing or still has placeholders: "The clinic hasn't been set up yet. Ask [supervising professor] to run `$legal-clinic:cold-start-interview` first."
...
Read from `config/local/codex-for-legal/legal-clinic/CLAUDE.md` and the ingested handbook. Cover, interactively:
...
|---|---|---|
| `$legal-clinic:client-intake` | Client interview | Formatted case summary with issues spotted, conflict flags, triage |
| `$legal-clinic:draft [doc type]` | Need a first draft of a common document | Practice-area template filled from case notes — *starting point, not final* |
| `$legal-clinic:memo` | Need to analyze a case internally | IRAC-format memo with research gaps flagged |
| `$legal-clinic:research-start [issue]` | Starting legal research | Roadmap: statutes to check, case law areas, search terms — *leads, not authoritative cites* |
| `$legal-clinic:status [audience]` | Updating someone on a case | Summary tailored to client / professor / court |
| `$legal-clinic:client-letter [type]` | Routine correspondence | Appointment confirm, doc request, status update from templates |
...
>
> Run `$legal-clinic:client-intake` and interview me as if I'm Maria. I'll answer as Maria would. At the end, look at the case summary it produces — what issues did it spot? Did it catch the habitability defense?
...
> Using Maria's intake, run `$legal-clinic:draft eviction-answer`. You'll get a first draft.
>
...
> Run `$legal-clinic:research-start "habitability defense to eviction in [state]"`. You'll get a roadmap — statutes, case law areas, search terms.
>
...
The point: `$legal-clinic:research-start` is a starting place, not a citation. The student still does the research.
...
- **Verify every citation** before it goes in anything. `$legal-clinic:research-start` gives leads, not authorities.
...
>
> The one-page reference card: `$legal-clinic:ramp --card`
