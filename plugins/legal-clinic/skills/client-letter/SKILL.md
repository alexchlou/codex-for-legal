...
1. Load `config/local/codex-for-legal/legal-clinic/CLAUDE.md` → plain-language standards, supervision style, clinic contact info.
...
Scope: routine only. Substantive advice → `$legal-clinic:status client` or a conversation with the professor.
...
**Scope: routine only.** Substantive advice, bad news, case strategy — those are `$legal-clinic:status client` or a conversation, not a template letter.

## Load context

`config/local/codex-for-legal/legal-clinic/CLAUDE.md` → plain-language standards, supervision style, clinic contact info.

## Pedagogy check

Read the supervisor guide for this practice area at `config/local/codex-for-legal/legal-clinic/guides/<practice-area>.md`. Check the `pedagogy_posture` setting:
...
For routine "we filed it" / "we're waiting" updates. (Fuller status updates → `$legal-clinic:status client`.)
...
Sending a letter to a client is a consequential action. This plugin's gate is the supervision workflow described in `## Supervision style` in `config/local/codex-for-legal/legal-clinic/CLAUDE.md`, reinforced by the Part 0 role check that confirms a licensed supervising attorney owns the clinic setup. That gate still holds: every letter clears review before it leaves the clinic.

Before sending any of the letters above, confirm:

1. The draft has been reviewed per the supervision protocol in `config/local/codex-for-legal/legal-clinic/CLAUDE.md` (queue / flag / lighter-touch).
...
Per `config/local/codex-for-legal/legal-clinic/CLAUDE.md` standards. Short sentences. No jargon. Reading level target enforced. If a template above includes a legal term the client might not know, explain it the first time: "We filed your 'answer' — that's the document that tells the court your side of the story."

## Supervision routing

Per `config/local/codex-for-legal/legal-clinic/CLAUDE.md`. Routine correspondence may or may not be a flag trigger depending on the supervision style the professor chose. If lighter-touch: these go out after student review without a queue step. If formal queue: even routine letters queue.

## What this skill does NOT do

- **Substantive advice.** If the letter would say "here's what I think about your case" or "here's what you should do," that's not routine — that's `$legal-clinic:status client` or a conversation with the professor first.
- **Bad news.** Case closing, adverse ruling, can't-help — those need thought, not a template. Flag for professor.
- **Anything to opposing counsel or a court.** Different audience, different skill (`$legal-clinic:draft` or `$legal-clinic:status court`).
