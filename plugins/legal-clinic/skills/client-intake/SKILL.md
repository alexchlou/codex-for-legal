...
1. Load `config/local/codex-for-legal/legal-clinic/CLAUDE.md` → practice areas, intake templates, supervision style, flag triggers.
...
Each practice area asks different questions. Use the template from `config/local/codex-for-legal/legal-clinic/CLAUDE.md` for this area. Defaults if none provided:
...
Per whatever conflict-check process `config/local/codex-for-legal/legal-clinic/CLAUDE.md` describes. At minimum:
...
Per `config/local/codex-for-legal/legal-clinic/CLAUDE.md` supervision style and flag triggers. If formal queue or configurable flags are enabled, and a trigger is present (deadline mentioned, DV indicator, immigration status at issue, etc.), note the flag.
