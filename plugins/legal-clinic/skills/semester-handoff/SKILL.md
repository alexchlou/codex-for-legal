...
1. Load `config/local/codex-for-legal/legal-clinic/CLAUDE.md` → clinic profile, semester dates, supervision style.
2. Load `config/local/codex-for-legal/legal-clinic/deadlines.yaml` and `config/local/codex-for-legal/legal-clinic/client-comms/[case-id]/log.md` per case.
...
5. Generate per-case handoff memo → `config/local/codex-for-legal/legal-clinic/handoffs/[semester]/[case_id].md`.
6. Generate cohort summary → `config/local/codex-for-legal/legal-clinic/handoffs/[semester]/_summary.md`.
...
Every semester, clinics lose their entire workforce and rebuild. `$legal-clinic:ramp` solves half the problem — it onboards the new cohort. This skill solves the other half: it offboards the departing cohort by producing handoff memos that capture what the next student needs to know about every active case.
...
- `config/local/codex-for-legal/legal-clinic/CLAUDE.md` → clinic profile, semester, practice areas, supervision style
- `config/local/codex-for-legal/legal-clinic/deadlines.yaml` → all active deadlines, grouped by case
- `config/local/codex-for-legal/legal-clinic/client-comms/[case-id]/log.md` (per case) → communications history
...
- Pull all active cases (from intake records + `config/local/codex-for-legal/legal-clinic/deadlines.yaml` case_ids + client-comms folders)
...
*Pulled from `config/local/codex-for-legal/legal-clinic/deadlines.yaml`. Incoming student's first job is to confirm these are accurate and owned.*
...
*From `config/local/codex-for-legal/legal-clinic/client-comms/[case-id]/log.md`. Three-line summary here; incoming student reads the full log.*
...
After all per-case memos, produce `config/local/codex-for-legal/legal-clinic/handoffs/[semester]/_summary.md`:
...
Closing a case or transitioning it to a new student is a consequential action. The gate is the supervision workflow in `## Supervision style` in `config/local/codex-for-legal/legal-clinic/CLAUDE.md`, reinforced by the Part 0 role check confirming a licensed supervising attorney owns the setup. Case-closing memos always get professor sign-off before the case is marked closed in the handoff document, regardless of supervision-style choice.

Per `config/local/codex-for-legal/legal-clinic/CLAUDE.md` supervision style:
...
Once reviewed, handoff memos live at `config/local/codex-for-legal/legal-clinic/handoffs/[semester]/[case_id].md`. The incoming student reads them during their `$legal-clinic:ramp` run at the start of next semester — `$legal-clinic:ramp` should surface the memos for cases the new student is assigned.

## Integration

- **`$legal-clinic:ramp`:** at the start of next semester, reads `config/local/codex-for-legal/legal-clinic/handoffs/[most-recent-semester]/` and surfaces per-case memos for the cases each new student is taking on.
- **`$legal-clinic:deadlines`:** feeds the pending-deadlines section of each memo.
- **`$legal-clinic:client-comms-log`:** feeds the communications history summary.
- **`$legal-clinic:supervisor-review-queue` (if formal review enabled):** handoff memos route here for professor approval.
