...
1. Load `config/local/codex-for-legal/litigation-legal/matters/[slug]/matter.md` → theory, pivot fact, key facts.
2. Load `config/local/codex-for-legal/litigation-legal/CLAUDE.md` → Document storage sources, default matter folder pattern.
3. Follow the workflow and reference below.
4. Identify sources in order: user-provided paths this session, default matter folder, declared sources from `config/local/codex-for-legal/litigation-legal/CLAUDE.md`.
...
8. Write `config/local/codex-for-legal/litigation-legal/matters/[slug]/chronology.md` (or format variant per flag).
...
- **`--matter` mode (default for in-house litigation counsel).** Matter-history-focused. Reads the matter's case theory and key facts from `matter.md`, pulls from declared document-storage sources (Google Drive (connector optional; not enabled in v1), SharePoint, Gmail, iManage (connector optional; not enabled in v1), CLM — whatever the `## Landscape` section of CLAUDE.md declares), and treats `history.md` as the running internal log (decisions, holds, reserve memos — intentionally not in the chronology). Output is matter-centric: what happened across the dispute, tagged for advocacy use.
...
- `config/local/codex-for-legal/litigation-legal/matters/[slug]/matter.md` → case theory, key facts, pivot fact (for significance tagging), key dates.
- Default matter folder pattern from CLAUDE.md → where docs for this slug live.

`--documents` mode also reads:
- eDiscovery platform metadata if a connector is available (Everlaw (connector optional; not enabled in v1), Relativity, DISCO, Aurora) — by custodian + date range.
- Bates-range manifest or production index if the user points at one.

**Conflicts gate — unbypassable (`--matter` mode).** Before building the chronology, check `config/local/codex-for-legal/litigation-legal/matters/_log.yaml` for the matter slug. If the matter is not in `_log.yaml`, refuse and route:
...
2. **eDiscovery connector** — if an MCP connector is available (Everlaw (connector optional; not enabled in v1), Relativity, DISCO, Aurora), pull by custodian + date range.
...
- **eDiscovery platforms (Everlaw (connector optional; not enabled in v1), Relativity, DISCO, Aurora)** — if connector is available, pull by custodian + date range; otherwise the user provides an export.
...
Location: `config/local/codex-for-legal/litigation-legal/matters/[slug]/chronology.md`. Complete, tagged, annotated. The reference doc counsel works from.
...
**Unreadable sources:**
[sources declared in CLAUDE.md but not accessible this run — e.g., "Everlaw (connector optional; not enabled in v1) production — no MCP connector; export needed"]
