...
3. If grand jury → stop, escalate per `config/local/codex-for-legal/litigation-legal/CLAUDE.md`. Otherwise continue.
4. Load `config/local/codex-for-legal/litigation-legal/matters/_log.yaml` for cross-check. Load `config/local/codex-for-legal/litigation-legal/CLAUDE.md` → landscape, privilege conventions, escalation norms.
...
7. Write `config/local/codex-for-legal/litigation-legal/inbound/[slug]/triage.md`. Copy or link subpoena to `config/local/codex-for-legal/litigation-legal/inbound/[slug]/incoming.[ext]`.
8. Hand off: `$litigation-legal:legal-hold --issue` if hold not in place; `$litigation-legal:matter-intake` if materiality warrants; `$litigation-legal:matter-briefing [slug]` if party subpoena in existing matter.
...
- `config/local/codex-for-legal/litigation-legal/matters/_log.yaml` — for related matter lookup and legal hold status
- `config/local/codex-for-legal/litigation-legal/CLAUDE.md` → landscape (regulators we deal with), house privilege conventions, escalation norms
...
- Privilege log will be required — flag the format per `config/local/codex-for-legal/litigation-legal/CLAUDE.md`
...
Output: `config/local/codex-for-legal/litigation-legal/inbound/[slug]/triage.md`.
...
**Privilege log format required:** [per `config/local/codex-for-legal/litigation-legal/CLAUDE.md`]
...
- [ ] Legal hold issued — [yes/no] — if no, run `$litigation-legal:legal-hold [slug] --issue` with subpoena scope
...
Every rule reference, case, statute, and regulation in this triage — including the Step 0 research citations, objection bases, and the privilege-log format pointer — is AI-generated and unverified. Before relying on any cite (especially in objections, a motion to quash, or correspondence with the issuing party), run a verification pass against a legal research tool (Westlaw (connector optional; not enabled in v1), CourtListener (connector optional; not enabled in v1), Trellis, Descrybe (connector optional; not enabled in v1), or your firm's platform) for accuracy, good law status, and local variants. Fabricated or misquoted citations in filed documents have resulted in sanctions. Source tags on each citation (e.g., `[Westlaw (connector optional; not enabled in v1)]`, `[web search — verify]`) show where it came from; `verify` tags carry higher fabrication risk and should be checked first.
```

### Step 9: Hand off

**Before responding to the subpoena (serving objections, producing documents, appearing for deposition, or filing a motion to quash — any substantive response to the issuing party or court):** Read `## Who's using this` in `config/local/codex-for-legal/litigation-legal/CLAUDE.md`. If the Role is Non-lawyer:
...
- If classified as **grand jury subpoena** → stop, flag for escalation per `config/local/codex-for-legal/litigation-legal/CLAUDE.md`, do not proceed with standard triage.
...
- If a legal hold isn't issued with subpoena scope, hand off to `$litigation-legal:legal-hold --issue` immediately.
