...
2. Otherwise: load `config/local/codex-for-legal/litigation-legal/matters/[slug]/matter.md` + log row.
3. Load `config/local/codex-for-legal/litigation-legal/CLAUDE.md` → privilege markings, hold template pointer, escalation norms.
...
- `config/local/codex-for-legal/litigation-legal/matters/_log.yaml` — log row (legal_hold fields + status)
- `config/local/codex-for-legal/litigation-legal/matters/[slug]/matter.md` — matter context (counterparty, facts, key custodians from internal_owners)
- `config/local/codex-for-legal/litigation-legal/CLAUDE.md` — house style for litigation hold template pointer, privilege marking, escalation norms
...
**Before issuing the hold to custodians (the consequential act):** Read `## Who's using this` in `config/local/codex-for-legal/litigation-legal/CLAUDE.md`. If the Role is Non-lawyer:
...
4. **Systems** — email, Slack (connector optional; not enabled in v1)/Teams, file shares, devices (including BYOD if applicable), Jira/Asana, CRM, legacy systems.
...
**Draft the notice** to each custodian, using the house template in `config/local/codex-for-legal/litigation-legal/CLAUDE.md` if one is configured; otherwise the default template below.
...
DATE: [effective date]
TO: [custodian name]
FROM: [signer — per `config/local/codex-for-legal/litigation-legal/CLAUDE.md` default]
...
1. All documents, emails, text messages, Slack (connector optional; not enabled in v1)/Teams messages, and other
...
- Slack (connector optional; not enabled in v1)/Teams/messaging platforms
...
**Writes:**
- `config/local/codex-for-legal/litigation-legal/matters/[slug]/legal-hold-v1.docx` via the `docx` skill
- Appends to `config/local/codex-for-legal/litigation-legal/matters/[slug]/history.md`:
  ```
...
**Writes:**
- `config/local/codex-for-legal/litigation-legal/matters/[slug]/legal-hold-v[N].docx` (next version number)
- `history.md` entry
...
**Before releasing the hold (the consequential act — preservation obligations resume normal retention):** Read `## Who's using this` in `config/local/codex-for-legal/litigation-legal/CLAUDE.md`. If the Role is Non-lawyer:
...
**Writes:**
- `config/local/codex-for-legal/litigation-legal/matters/[slug]/legal-hold-release.docx`
- `history.md` entry
...
This is a separate command invocation (`$litigation-legal:legal-hold --status` with no slug) OR invoked by `$litigation-legal:portfolio-status` as a section in the portfolio rollup.

## Integration with portfolio-status

The `portfolio-status` skill already flags "Hold not issued on active litigation." This skill is what resolves those flags. Worth cross-referencing in the briefing when a matter is opened: if `legal_hold.issued == false`, `$litigation-legal:matter-intake` closes by offering to run `$litigation-legal:legal-hold --issue`.
