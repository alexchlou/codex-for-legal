...
1. Load `config/local/codex-for-legal/litigation-legal/CLAUDE.md` → demand-letter practice, landscape, risk calibration.
...
5. Write `config/local/codex-for-legal/litigation-legal/demand-letters/[slug]/intake.md`.
...
- Desired outcome dollar value ≥ the medium-severity band from `config/local/codex-for-legal/litigation-legal/CLAUDE.md` risk calibration
- Counterparty is a customer, competitor, or frequent adversary per `config/local/codex-for-legal/litigation-legal/CLAUDE.md` landscape
- User ran with `--full`
...
The draft skill honors the skip — pre-draft gate runs regardless, but sections that depend on strategic-block answers get `[SME VERIFY: leverage/tone/privilege not captured in intake]` markers. The `$litigation-legal:demand-draft` command also prompts a second time, asking whether the user wants to complete the strategic block before drafting.
...
`[type]-[counterparty-short]-[yyyy-mm]`. Confirm uniqueness in `config/local/codex-for-legal/litigation-legal/demand-letters/`.

### `config/local/codex-for-legal/litigation-legal/demand-letters/[slug]/intake.md`
...
- Run the conflicts check. If the counterparty is a customer or known entity, flag that this should clear conflicts (per `config/local/codex-for-legal/litigation-legal/CLAUDE.md`) before sending — but the check itself lives in the matter-intake workflow or outside this skill.
