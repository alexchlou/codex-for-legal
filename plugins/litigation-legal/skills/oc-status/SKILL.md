...
1. Load `config/local/codex-for-legal/litigation-legal/matters/_log.yaml`, filter per default rules (or per flags).
2. Load `config/local/codex-for-legal/litigation-legal/CLAUDE.md` → outside counsel directive style, signer defaults, budget posture.
3. Follow the workflow and reference below.
4. For each matter in scope: read `matter.md` + `history.md`, draft per-matter email.
5. Write markdown to `config/local/codex-for-legal/litigation-legal/oc-status/[YYYY-MM-DD]/[slug].md`.
6. If Gmail MCP authenticated: create Gmail drafts. Else: markdown-only, note in summary.
7. Write `config/local/codex-for-legal/litigation-legal/oc-status/[YYYY-MM-DD]/_summary.md` — what ran, what was skipped and why.
...
- `config/local/codex-for-legal/litigation-legal/matters/_log.yaml` — the filtering and field source
- `config/local/codex-for-legal/litigation-legal/matters/[slug]/matter.md` — matter context (current posture, open questions)
- `config/local/codex-for-legal/litigation-legal/matters/[slug]/history.md` — recent events to inform what to ask about
- `config/local/codex-for-legal/litigation-legal/CLAUDE.md` → outside counsel directive style, signer name/email, budget posture
...
**Subject:** per house convention (from `config/local/codex-for-legal/litigation-legal/CLAUDE.md` outside counsel directive style; fallback: `[Matter: [matter name]] — Weekly status update`)
...
4. **Budget** — [monthly / quarterly / on-request per `config/local/codex-for-legal/litigation-legal/CLAUDE.md` budget posture]. Where are we against [budget authorization from matter.md]? Any variance to flag?
...
[Signoff — name, role, contact. From `config/local/codex-for-legal/litigation-legal/CLAUDE.md` signer default for OC directives.]
```

Adapt tone per `config/local/codex-for-legal/litigation-legal/CLAUDE.md` outside counsel directive style — some shops are "dear counsel" formal; others are first-name-and-bullets. Match.

## Output

### Markdown drafts

Write to: `config/local/codex-for-legal/litigation-legal/oc-status/[YYYY-MM-DD]/[slug].md`
...
**From:** [signer name / email from `config/local/codex-for-legal/litigation-legal/CLAUDE.md`]
**Subject:** [subject line]

> The work-product header above applies to this internal record. The outgoing email body below goes to outside counsel on a retained matter, which is itself a privileged communication — apply the house privilege marking (`config/local/codex-for-legal/litigation-legal/CLAUDE.md` privilege conventions) at the top of the email sent, typically `Privileged & Confidential — Attorney-Client Communication / Attorney Work Product`, not this internal work-product header.
...
After processing all matters, write `config/local/codex-for-legal/litigation-legal/oc-status/[YYYY-MM-DD]/_summary.md`:
...
| [slug] | no OC email in log — update with `$litigation-legal:matter-update [slug]` |
...
Ad-hoc: `$litigation-legal:oc-status` any time. `$litigation-legal:oc-status --slug=foo` for a single matter.
...
- **Rewrite history.md.** Reads it for context; doesn't modify. (If OC's response surfaces new events, use `$litigation-legal:matter-update [slug]` to log them.)
- **Enforce a minimum template.** If the house tone is "one line, first name, done," the draft honors that and skips the bulleted structure. Match `config/local/codex-for-legal/litigation-legal/CLAUDE.md`.
