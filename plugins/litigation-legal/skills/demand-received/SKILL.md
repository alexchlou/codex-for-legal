...
2. Load `config/local/codex-for-legal/litigation-legal/matters/_log.yaml` for portfolio cross-check.
3. Load `config/local/codex-for-legal/litigation-legal/CLAUDE.md` → risk calibration, landscape, demand-letter practice.
...
6. Write `config/local/codex-for-legal/litigation-legal/inbound/[slug]/triage.md`. Copy or link incoming to `config/local/codex-for-legal/litigation-legal/inbound/[slug]/incoming.[ext]`.
...
- `config/local/codex-for-legal/litigation-legal/matters/_log.yaml` — scan for related matters (same counterparty, overlapping counterparties via entity relationships, or matter type + recent date)
- `config/local/codex-for-legal/litigation-legal/CLAUDE.md` → risk calibration (for merit assessment), landscape (is the sender a frequent adversary?), demand-letter practice (house tone and response defaults)
...
- **Leverage and pressure** — are they credibly prepared to sue? Do they have capacity? Are they a repeat-litigant adversary per `config/local/codex-for-legal/litigation-legal/CLAUDE.md`?
...
- Next step: `$litigation-legal:demand-intake` with pre-populated fields for a counter-response letter
...
- Next step: `$litigation-legal:demand-intake` with `type: settlement-response`
...
- Next step: issue legal hold via `$litigation-legal:legal-hold --issue` if not already; log the demand and move on
...
- [ ] Legal hold issued — [yes/no] — if no, run `$litigation-legal:legal-hold [slug] --issue`
...
- Matter creation → hand off to `$litigation-legal:matter-intake` with: counterparty, type, `source: demand-letter` (inbound), initial theory framed defensively, pre-populated.
- Counter-response as outbound demand → hand off to `$litigation-legal:demand-intake` with: counterparty, context from triage, desired outcome as the response.
- Link to existing matter → update that matter's `related_matters` in `_log.yaml`; append event to its `history.md`.
- Standalone → leave in `config/local/codex-for-legal/litigation-legal/inbound/`; no portfolio change.
