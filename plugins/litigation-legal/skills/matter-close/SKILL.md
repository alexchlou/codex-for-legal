...
5. Append final entry to `config/local/codex-for-legal/litigation-legal/matters/[slug]/history.md`.
6. Matter stays in `_log.yaml` and `config/local/codex-for-legal/litigation-legal/matters/[slug]/` — not deleted. `$litigation-legal:portfolio-status` filters it from active rollups.
...
**Before closing the matter (the consequential act — the matter is archived and active tracking ends):** Read `## Who's using this` in `config/local/codex-for-legal/litigation-legal/CLAUDE.md`. If the Role is Non-lawyer:

> Closing a matter has legal consequences — it ends active tracking, may affect any associated legal hold (run `$litigation-legal:legal-hold --release` separately if appropriate), and establishes the final record the company relies on. Have you reviewed this with an attorney? If yes, proceed. If no, here's a brief to bring to them:
>
...
### Append final entry to `config/local/codex-for-legal/litigation-legal/matters/[slug]/history.md`
...
### Touch `config/local/codex-for-legal/litigation-legal/matters/[slug]/matter.md`
