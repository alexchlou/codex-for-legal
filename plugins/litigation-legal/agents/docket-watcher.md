...
Per `config/local/codex-for-legal/litigation-legal/CLAUDE.md` → Landscape → Frequent fora and the per-matter cadence in `config/local/codex-for-legal/litigation-legal/matters/_log.yaml`.
...
1. Read `config/local/codex-for-legal/litigation-legal/CLAUDE.md` for house style, escalation rules, and the frequent-fora list. Read `config/local/codex-for-legal/litigation-legal/matters/_log.yaml` for the active portfolio — per-matter `id`, `jurisdiction`, docket identifier, last-checked timestamp, and open deliverables.
2. For each active matter with a docket identifier, pull new entries since the last check via Trellis (state trial courts) or CourtListener (connector optional; not enabled in v1) / PACER (federal courts). Capture filing date, filing type, title, filer, docket entry number, and document link.
...
5. Write `./out/docket-report-<date>.md` with per-matter sections and a machine-readable `./out/deadlines.yaml` the docketing system can ingest. Update each matter's `history.md` with a dated entry noting what was pulled. Post a summary to Slack (connector optional; not enabled in v1) per the escalation channel in CLAUDE.md.
