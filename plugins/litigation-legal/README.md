...
Your configuration is stored at `config/local/codex-for-legal/litigation-legal/CLAUDE.md` and survives plugin updates.

## Commands

| Command | Does |
|---|---|
| `$litigation-legal:cold-start-interview` | Cold-start → writes house `config/local/codex-for-legal/litigation-legal/CLAUDE.md` |
...
Your practice profile at `config/local/codex-for-legal/litigation-legal/CLAUDE.md` isn't static — it improves as you use the plugin. Skills tell you when an output used a default you should tune. You can re-run setup, edit the file directly, or tell a skill to record a new position.

## Notes

- Every skill reads from `config/local/codex-for-legal/litigation-legal/CLAUDE.md` first. If your risk appetite changes or you bring on new outside counsel, update it — don't paper over it in individual matters.
- `## Company profile` is the first section of `config/local/codex-for-legal/litigation-legal/CLAUDE.md` by convention. If you run other `-legal` plugins, you can copy it across rather than re-entering the same context.
