...
description: >
  ...
  `config/local/codex-for-legal/commercial-legal/CLAUDE.md`. Flags deviations, assesses risk, generates
...
Read a vendor agreement against the playbook this team actually uses (in `config/local/codex-for-legal/commercial-legal/CLAUDE.md`), find every term that deviates, and tell the lawyer what to do about each one — with specific redline language, not vague "consider revising."
...
**Before reading the contract, read `config/local/codex-for-legal/commercial-legal/CLAUDE.md`.** If it's missing or still has placeholders, surface this bounce:
...
The playbook in `config/local/codex-for-legal/commercial-legal/CLAUDE.md` is the source of truth. It tells you:
...
Check the "one thing" from `config/local/codex-for-legal/commercial-legal/CLAUDE.md` first. If present:
...
For each playbook category in `config/local/codex-for-legal/commercial-legal/CLAUDE.md`, find the corresponding contract section and compare.
...
**Playbook says:** [our standard position, quoted from `config/local/codex-for-legal/commercial-legal/CLAUDE.md`]
...
**If they won't move:** [the fallback from `config/local/codex-for-legal/commercial-legal/CLAUDE.md`, or "escalate to [person]"
...
| Level | Means |
|---|---|
| 🔴 Critical | Don't sign without fixing. A term on the team's "never accept" list in `config/local/codex-for-legal/commercial-legal/CLAUDE.md`, or a deal-breaker. |
...
Severity is always applied *against `config/local/codex-for-legal/commercial-legal/CLAUDE.md`*. If a term doesn't map cleanly to a playbook position, ask the user which bucket it belongs in and offer to record the answer in `config/local/codex-for-legal/commercial-legal/CLAUDE.md`.
...
Check the escalation matrix in `config/local/codex-for-legal/commercial-legal/CLAUDE.md` against:
...
**Before proceeding to send redlines to the counterparty:** Read `## Who's using this` in `config/local/codex-for-legal/commercial-legal/CLAUDE.md`. If the Role is Non-lawyer:
...
>
> **Source attribution.** Where the memo cites a statute, regulation, or case, tag the citation: `[Westlaw (connector optional; not enabled in v1)]`, `[statute / regulator site]`, or the MCP tool name for citations retrieved from a legal research connector; `[web search — verify]` for web-search citations; `[model knowledge — verify]` for citations recalled from training data; `[user provided]` for citations from the counterparty draft or house files. Citations tagged `verify` carry higher fabrication risk and should be checked first. Never strip or collapse the tags.
...
## Integration: DocuSign (connector optional; not enabled in v1)

If DocuSign (connector optional; not enabled in v1) MCP is connected and the agreement is ready to sign (all greens or all issues accepted), offer to:
...
**Before generating a signature envelope or routing for countersignature:** Read `## Who's using this` in `config/local/codex-for-legal/commercial-legal/CLAUDE.md`. If the Role is Non-lawyer:
...
**Full memo (default):** As above. Goes in the [CLM] record or the Drive folder from `config/local/codex-for-legal/commercial-legal/CLAUDE.md` house-style section.

**Slack (connector optional; not enabled in v1)-sized summary:** Two lines and a link. For when someone asks "is this okay?" in a channel.
...
- [ ] `config/local/codex-for-legal/commercial-legal/CLAUDE.md` was loaded and quoted — not generic market positions
