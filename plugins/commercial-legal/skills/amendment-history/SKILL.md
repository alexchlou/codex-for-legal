...
argument-hint: "[file(s) | [local CLM export or pasted text (CLM connector required for IDs) (coming soon)] | [repository link (coming soon)]] [--provision <clause name>]"
---
...
1. **Get the documents:** From file upload, [local CLM export or pasted text (CLM connector required for IDs) (coming soon)], or [repository link (coming soon)]. Accept multiple files in one invocation. If none
   provided, ask.
...
This skill reads the base agreement and amendments — often privileged or confidential in their own right, and typically used for privileged analysis. The output inherits the source's privilege and confidentiality status. Prepend the work-product header from `config/local/codex-for-legal/commercial-legal/CLAUDE.md` `## Outputs` to every output below, distribute only within the privilege circle, and store it where privileged materials live. Strip the header before any external delivery.
...
- It does not compare against the playbook in `config/local/codex-for-legal/commercial-legal/CLAUDE.md` — that is the
