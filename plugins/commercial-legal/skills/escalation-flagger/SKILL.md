...
description: >
  ...
  `config/local/codex-for-legal/commercial-legal/CLAUDE.md`, and draft the ask. Use when the user
...
Names the approver for a contract issue per the `config/local/codex-for-legal/commercial-legal/CLAUDE.md` escalation matrix and drafts the message so you're not writing "hey got a sec" at 5pm.

## Instructions

1. **Load `config/local/codex-for-legal/commercial-legal/CLAUDE.md`** → Escalation section. If missing, say so — the practice profile needs editing.
...
Every contracts team has an escalation matrix, written or not. This skill reads the written one (in `config/local/codex-for-legal/commercial-legal/CLAUDE.md`), matches a contract issue against it, names the approver, and drafts the ask so the lawyer isn't writing "hey do you have a sec" messages at 5pm.
...
Read `config/local/codex-for-legal/commercial-legal/CLAUDE.md` → `## Escalation`. If it's missing or vague, say so — the cold-start interview should have captured this, and if it didn't, the practice profile needs editing.

Expected structure:

| Can approve | Threshold | Escalates to | Via |
|---|---|---|---|
| Paralegal | Standard terms, <$50K | Counsel | Slack (connector optional; not enabled in v1) |
| Counsel | Non-standard but within fallbacks, <$500K | GC | Slack (connector optional; not enabled in v1) or email |
...
Don't escalate things that are actually fine. If the term is within the fallbacks in `config/local/codex-for-legal/commercial-legal/CLAUDE.md`, it doesn't need to go up.
...
Be specific. Not "escalate to legal leadership" — name the person or role from `config/local/codex-for-legal/commercial-legal/CLAUDE.md`. If the matrix doesn't name anyone for this situation, say so: "The escalation matrix doesn't cover [situation]. Suggest asking [GC name] who owns this."
...
```markdown
**Escalating to:** [name]
**Via:** [Slack (connector optional; not enabled in v1) #channel / email / meeting — per `config/local/codex-for-legal/commercial-legal/CLAUDE.md`]
...
**What our playbook says:** [quote from `config/local/codex-for-legal/commercial-legal/CLAUDE.md`]
...
The calibration for what warrants escalation lives in `config/local/codex-for-legal/commercial-legal/CLAUDE.md`, not in this skill. Check the playbook's stated position, its fallbacks, and its "automatic escalation regardless of dollar value" list:
...
If a term comes up that the playbook doesn't address, don't guess the threshold — ask the reviewing attorney whether this class of issue should escalate, and offer to record the answer in `config/local/codex-for-legal/commercial-legal/CLAUDE.md` so future reviews are consistent.
