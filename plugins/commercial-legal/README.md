...
It writes what it learns to `config/local/codex-for-legal/commercial-legal/CLAUDE.md` — a plain-English document about your team that every other skill reads before doing anything. You edit the document, not a config file.
...
| Skill | Purpose |
|---|---|
| **cold-start-interview** | First-run interview that writes `config/local/codex-for-legal/commercial-legal/CLAUDE.md` |
...
- **Ironclad (connector optional; not enabled in v1)** — contract lifecycle management
- **DocuSign (connector optional; not enabled in v1)** — signature status and envelope tracking
...
With DocuSign (connector optional; not enabled in v1) connected: track signature status, route envelopes in approver order.
...
Your configuration is stored at `config/local/codex-for-legal/commercial-legal/CLAUDE.md` and survives plugin updates.
