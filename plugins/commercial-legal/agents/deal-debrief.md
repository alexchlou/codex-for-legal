...
The output feeds `config/local/codex-for-legal/commercial-legal/deviation-log.yaml`. The playbook-monitor agent reads that log to propose playbook updates when patterns emerge — but only from deals the attorney hasn't flagged as one-offs.
...
Read `config/local/codex-for-legal/commercial-legal/CLAUDE.md` in full. Extract:
...
Using the repository location from `config/local/codex-for-legal/commercial-legal/CLAUDE.md`:
...
- **If Google Drive (connector optional; not enabled in v1) / SharePoint:** search the specified folder for documents created or modified in the last 7 days with execution indicators (signatures present, "executed" in filename or metadata).
...
2. Identify the applicable playbook section(s) from `config/local/codex-for-legal/commercial-legal/CLAUDE.md`.
...
```
[#] [Deal] — [Clause]
Playbook position: [standard position from `config/local/codex-for-legal/commercial-legal/CLAUDE.md`]
...
Append a structured entry to `config/local/codex-for-legal/commercial-legal/deviation-log.yaml` for each agreement processed.

For agreements with deviations:

```yaml
- deal_id: [local CLM export or pasted text (CLM connector required for IDs) if available; otherwise auto-generate as YYYYMMDD-counterparty-slug]
  counterparty: [name]
...
Logged to: config/local/codex-for-legal/commercial-legal/deviation-log.yaml
