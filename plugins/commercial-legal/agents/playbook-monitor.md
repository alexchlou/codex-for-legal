> Codex migration note: This agent file is retained as a manual workflow description. Do not create a recurring Codex automation or external monitor unless the user explicitly asks for one.
...
description: >
  ...
  within a rolling 12-month window (configurable in `config/local/codex-for-legal/commercial-legal/CLAUDE.md`).
...
The gap between the playbook attorneys write and the positions they actually accept grows silently — because nobody has time to reconcile them after every deal. This agent watches the deviation log, detects when a position is being overridden consistently, and proposes a specific update to `config/local/codex-for-legal/commercial-legal/CLAUDE.md`. The attorney approves or rejects. The playbook stays alive.
...
Both values are configurable in `config/local/codex-for-legal/commercial-legal/CLAUDE.md` under `## Playbook monitor settings`:
...
If these fields are absent from `config/local/codex-for-legal/commercial-legal/CLAUDE.md`, use the defaults above.

## What it does

### Step 1 — Read the practice profile and log

1. Read `config/local/codex-for-legal/commercial-legal/CLAUDE.md` in full. Extract:
   ...
   - Notification destination (Slack (connector optional; not enabled in v1) channel or email from House style section)

2. Read `config/local/codex-for-legal/commercial-legal/deviation-log.yaml`. Filter out:
...
If no clause crosses the threshold: log the check to `config/local/codex-for-legal/commercial-legal/playbook-monitor-log.yaml` and stop. Do not notify the attorney.
...
2. **Current playbook language** (exact text from `config/local/codex-for-legal/commercial-legal/CLAUDE.md`)
...
Current language in `config/local/codex-for-legal/commercial-legal/CLAUDE.md`:
...
Write all proposals to `config/local/codex-for-legal/commercial-legal/playbook-proposals.md`. Overwrite any existing file — stale unreviewed proposals are replaced, not accumulated.
...
Notify the attorney via the destination in `config/local/codex-for-legal/commercial-legal/CLAUDE.md`:
...
> Proposals: config/local/codex-for-legal/commercial-legal/playbook-proposals.md

Log the run to `config/local/codex-for-legal/commercial-legal/playbook-monitor-log.yaml`:
...
- run_at: [ISO datetime]
  ...
  proposals_file: config/local/codex-for-legal/commercial-legal/playbook-proposals.md
```

### Step 5 — Review and approval (triggered by $commercial-legal:review-proposals command)

When the attorney runs `$commercial-legal:review-proposals`:

1. Read `config/local/codex-for-legal/commercial-legal/playbook-proposals.md`. If file doesn't exist or is empty: *"No pending proposals. Playbook is up to date."* Stop.
...
[A] Accept — apply proposed language to `config/local/codex-for-legal/commercial-legal/CLAUDE.md`
...
```
Updating `config/local/codex-for-legal/commercial-legal/CLAUDE.md`:
...
5. **Reject / Defer:** log to `config/local/codex-for-legal/commercial-legal/playbook-monitor-log.yaml` with reason if given. Do not modify `config/local/codex-for-legal/commercial-legal/CLAUDE.md`. A rejected proposal is not re-raised until a new pattern emerges after the rejection date.

6. After all proposals resolved, show summary:

```
Review complete.
[N] accepted and applied to `config/local/codex-for-legal/commercial-legal/CLAUDE.md`
[N] rejected
[N] deferred to next cycle
[N] edited and applied

`config/local/codex-for-legal/commercial-legal/CLAUDE.md` last updated: [timestamp]
Next playbook check: after [N] more deals are logged
```

7. Archive: rename `config/local/codex-for-legal/commercial-legal/playbook-proposals.md` to `config/local/codex-for-legal/commercial-legal/playbook-proposals-[YYYYMMDD].md`. The active file is now clear.

## What this agent does NOT do

- Modify `config/local/codex-for-legal/commercial-legal/CLAUDE.md` without explicit per-change attorney confirmation
