...
1. **Read the allowlist first.** `config/local/codex-for-legal/legal-builder-hub/allowlist.yaml`. If restrictive mode and source not listed: refuse. If permissive: warn and continue.
...
7. **Install.** Copy the directory. Update `config/local/codex-for-legal/legal-builder-hub/CLAUDE.md` and append to `install-log.yaml`.
...
Read `config/local/codex-for-legal/legal-builder-hub/allowlist.yaml`.
...
- Instructions to read files outside `config/local/` or the skill's
  own directory
...
Record in `config/local/codex-for-legal/legal-builder-hub/CLAUDE.md`
