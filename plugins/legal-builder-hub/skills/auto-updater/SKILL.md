...
1. Load `config/local/codex-for-legal/legal-builder-hub/CLAUDE.md` → installed skills + auto-update prefs.
...
`config/local/codex-for-legal/legal-builder-hub/CLAUDE.md` → installed skills (with version/commit SHA), update preferences (notify / manual).
...
Replace the installed skill files with the new version. Update `config/local/codex-for-legal/legal-builder-hub/CLAUDE.md` installed list with the new commit SHA. Backup the old version first (to `~/.claude/skills/.backups/[skill]-[old-sha]/`) in case of rollback.
