...
- The skill's files must resolve to a path outside the built-in plugin
  directories that ship with codex-for-legal.
...
The 12 core plugins that ship with codex-for-legal are off-limits from this
...
wrote to the user's `config/local/...` — surface this to the user
...
- Uninstall first-party plugin skills. Use the Codex Plugin Directory or `codex plugin marketplace` commands for plugin management.
- Delete user configuration by default. Configs in
  `config/local/codex-for-legal/<plugin>/` are preserved unless
