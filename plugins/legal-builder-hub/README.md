...
Your configuration is stored at `config/local/codex-for-legal/legal-builder-hub/CLAUDE.md` and survives plugin updates.
...
- Slack (connector optional; not enabled in v1) notifications from the registry-sync agent require a Slack (connector optional; not enabled in v1) MCP server configured in your environment. Without one, the agent writes its digest to a file.
- The default registry list in `config/local/codex-for-legal/legal-builder-hub/CLAUDE.md` ships empty except for `lpm-skills`. Add registries you trust via `$legal-builder-hub:registry-browser` or by editing `config/local/codex-for-legal/legal-builder-hub/CLAUDE.md`.
...
The default allowlist ships with the community registries we've reviewed pre-configured. Edit `references/allowlist-default.yaml` in the repo, or your per-install allowlist at `config/local/codex-for-legal/legal-builder-hub/allowlist.yaml`, to add, remove, or switch between restrictive and permissive modes.
