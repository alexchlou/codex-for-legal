...
1. Load `config/local/codex-for-legal/regulatory-legal/CLAUDE.md` → policy library index + practice profile.
...
One of:
- A `GAP-ID` from the gap tracker — load the entry from `config/local/codex-for-legal/regulatory-legal/gap-tracker.yaml` (or the matter-level equivalent).
...
This skill reads the policy library index and owners from `config/local/codex-for-legal/regulatory-legal/CLAUDE.md`. When a value it needs is empty or still `[PLACEHOLDER]`:
