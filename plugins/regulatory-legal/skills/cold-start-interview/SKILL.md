...
1. Check `config/local/codex-for-legal/regulatory-legal/CLAUDE.md`. If a populated CLAUDE.md (no `[PLACEHOLDER]` markers) exists at `config/local/cache/codex-for-legal/regulatory-legal/*/CLAUDE.md` but not at the config path, copy it to the config path and tell the user what was migrated. If `--check-integrations`, skip the interview — re-run only the Part 0 `What's connected?` check and rewrite the `## Available integrations` table in `config/local/codex-for-legal/regulatory-legal/CLAUDE.md`.
...
4. Write `config/local/codex-for-legal/regulatory-legal/CLAUDE.md` (creating parent directories as needed) with watchlist + materiality threshold.
...
Read `config/local/codex-for-legal/regulatory-legal/CLAUDE.md`:
...
If a CLAUDE.md exists at the old cache path `config/local/cache/codex-for-legal/regulatory-legal/*/CLAUDE.md` but not at the config path, copy it forward.
...
- **Pause and resume.** Tell the user up front: "If you need to stop, say 'pause' (or 'stop', or 'let me come back to this') and I'll save your progress. Run `$regulatory-legal:cold-start-interview` again later and I'll pick up where you left off." When the user pauses, write a partial configuration to `config/local/codex-for-legal/regulatory-legal/CLAUDE.md` with a `<!-- SETUP PAUSED AT: [section name] — run $regulatory-legal:cold-start-interview to resume -->` comment at the top and `[PENDING]` markers (distinct from `[PLACEHOLDER]`) on unanswered fields. When setup re-runs and finds a paused config, greet the user: "Welcome back. You paused at [section]. Your earlier answers are saved. Pick up where we left off, or start over?" Do not re-ask questions already answered.
...
> This plugin can work with: regulatory feeds (Thomson Reuters Regulatory Intelligence), document storage (Google Drive (connector optional; not enabled in v1), SharePoint, Box), and Slack (connector optional; not enabled in v1). Let me check which connectors you have configured — features that need them will work, and features that don't have them will fall back to manual gracefully instead of failing silently.
...
- CourtListener (connector optional; not enabled in v1)? Which trackers?
...
| CourtListener (connector optional; not enabled in v1) | [yes/no] | [tracker names] |
...
- Close with the changeability note:
  ...
  > "Done. Your configuration is at `config/local/codex-for-legal/regulatory-legal/CLAUDE.md` — a plain-text file you can read and edit directly. Anything you answered can be changed:
  >
