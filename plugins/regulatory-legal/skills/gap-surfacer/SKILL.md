...
description: >
  ...
  and notifies gap owners via Slack (connector optional; not enabled in v1) with per-send confirmation. Loaded by the gaps
...
Before sending ANY Slack (connector optional; not enabled in v1) message (assignment notice, overdue reminder, bulk notification, status report):
...
Lives at `config/local/codex-for-legal/regulatory-legal/gap-tracker.yaml`:

> **Note on comment-tracker.yaml:** `config/local/codex-for-legal/regulatory-legal/comment-tracker.yaml` is a sibling file owned by the comments skill. It is written to by reg-feed-watcher (which logs NPRMs automatically) and the comments skill (which tracks user-initiated comment decisions). This skill does not read or cross-reference it. If you modify the comment-tracker schema, update both actual consumers.
...
gaps:
  - id: GAP-001
    ...
    owner_slack: "[Slack (connector optional; not enabled in v1) user ID or handle, if known]"
    opened: 2026-03-01
...
If Slack (connector optional; not enabled in v1) MCP is available and `owner_slack` is set:

Send a Slack (connector optional; not enabled in v1) DM to the gap owner — but only after the per-send confirmation at the top of this file. Preview the message to the user, wait for an explicit yes, then send:
...
If Slack (connector optional; not enabled in v1) MCP is not available: note in the status report that owner notification
...
- **Escalation path missing for an overdue material gap:** still report it as overdue, and append: "No escalation path is set for material overdue gaps. Configure it with `$regulatory-legal:cold-start-interview --redo` or by editing `config/local/codex-for-legal/regulatory-legal/CLAUDE.md`."
...
- `partial`, `full`, `new-policy`, `none`: if due date is within 30 days and a reminder has not been sent in the last 7 days, PREVIEW a Slack (connector optional; not enabled in v1) DM (subject "⏰ Reminder: compliance gap due in [N] days") and wait for per-send confirm before sending.
- `comment-decision`: if comment deadline is within 21 days and a reminder has not been sent in the last 7 days, PREVIEW a Slack (connector optional; not enabled in v1) DM (subject "💬 Comment-decision deadline in [N] days") and wait for per-send confirm before sending.
- `watch`: no auto-reminder. Revisit when the tracker is reviewed or an NPRM is logged for the same regulation.
- If due date has passed on a compliance gap: flag as overdue in the report and PREVIEW a Slack (connector optional; not enabled in v1) DM — wait for per-send confirm before sending.
- If comment deadline has passed on a `comment-decision` item and no comment was filed: flag as overdue, PREVIEW a Slack (connector optional; not enabled in v1) DM (wait for per-send confirm), and ask the owner to update to `risk-accepted` (deliberate no-comment) or `closed` (comment filed) with a note.
...
**Before closing a gap as resolved, or producing any output that certifies compliance with a regulatory requirement (internal attestation, board report, audit response, regulator response):** Read `## Who's using this` in config/local/codex-for-legal/regulatory-legal/CLAUDE.md. If the Role is **Non-lawyer**:
...
due-date reminder check and sends any outstanding Slack (connector optional; not enabled in v1) notifications.
...
- Send Slack (connector optional; not enabled in v1) notifications if the Slack (connector optional; not enabled in v1) MCP is not configured. Falls back to
