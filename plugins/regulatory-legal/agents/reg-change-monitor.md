...
description: >
  ...
  Runs per the cadence in config/local/codex-for-legal/regulatory-legal/CLAUDE.md. Filters by materiality threshold so the
...
Per `config/local/codex-for-legal/regulatory-legal/CLAUDE.md` → Feed configuration → Check cadence. Default weekly; daily if the regulatory environment is active.

## What it does

1. Read `config/local/codex-for-legal/regulatory-legal/CLAUDE.md` → watchlist, materiality threshold.
