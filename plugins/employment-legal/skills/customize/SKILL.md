...
2. **Show the customizable map.** List what's in the profile, grouped, with a
   ...
   - **Workflow** — matter workspaces, leave tracker cadence, expansion
     project paths
   - **Integrations** — HRIS / Slack (connector optional; not enabled in v1) / document storage status, fallbacks
...
4. **Make the change.** Show the current value, ask for the new value, explain
   ...
   Examples:
   - *Adding Washington to the jurisdictional footprint:* "`$employment-legal:wage-hour-qa`
     and `$employment-legal:termination-review` will start applying WA rules. `/handbook-
     updates` will prompt for a WA supplement. `$employment-legal:hiring-review` will now
