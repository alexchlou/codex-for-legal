...
Read `config/local/codex-for-legal/employment-legal/CLAUDE.md` → jurisdictional footprint, any classification history or
prior settlements noted, escalation table, and any house classification
policy the team has recorded.

## Output header

Prepend the work-product header from `config/local/codex-for-legal/employment-legal/CLAUDE.md` → `## Outputs` (it differs by user role — see `## Who's using this`).
...
If `config/local/codex-for-legal/employment-legal/CLAUDE.md` records the company's house classification policy, apply it
...
Escalate per `config/local/codex-for-legal/employment-legal/CLAUDE.md` if any of the following, or any team-specific
...
entity] — coordinate with them on worker agreement. No `$employment-legal:hiring-review` needed."]
```

## Consequential-action gate (classify a worker)

**Before producing a "Proceed as IC / employee / agency / vendor" final recommendation:** Read `## Who's using this` in `config/local/codex-for-legal/employment-legal/CLAUDE.md`. If the Role is **Non-lawyer**:
